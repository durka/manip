...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[scale=1,tdplot_main_coords]
    ...
    % prismatic joint
    \draw[dashed,red]   (0,0,0)   -- (-0.75,1.5,1.5) node[anchor=south,black] {\small $\bar{o}$};
    \draw[dashed,red] (0,0,0) -- (-0.75,1.5,0) -- (-0.75,1.5,1.5);
    \draw[->,>=*]    (-0.75,1.5,1.5) -- (-0.75,1.5,1.5);

    \node[align=center] at (2,1) {$\bar{x} = \bar{o}$ \\ Rigid};
  \end{tikzpicture}
  \linebreak
  \hfill
  \begin{tikzpicture}[scale=1,tdplot_main_coords]
    ...
    \draw[dashed,red]   (0,0,0)   -- (-0.75,1.5,1.5) node[anchor=south,black] {\small $\bar{o}$};
    \draw[dashed,red] (0,0,0) -- (-0.75,1.5,0) -- (-0.75,1.5,1.5);
    \draw[->,>=*]    (-0.75,1.5,1.5) -- (-0.5,2.5,1.5) node[below=-1] {\small $\vec{u}$};
    \draw[thick,<->,blue] (-0.65,2.24,1.75) -- node[above=-2,black] {\small $e$} (-0.48,2.8,1.75);

    \node[align=center] at (2,1) {$\bar{x} = T(e\vec{u})*\bar{o}$ \\ Prismatic};
  \end{tikzpicture}
  \hfill
  \begin{tikzpicture}[scale=1,tdplot_main_coords]
    \coordinate (O) at (0,0,0);
    ...
    % revolute joint
    \draw[dashed,red]   (0,0,0)   -- (-0.5,1.5,2) node[anchor=south,black] {\small $\bar{c}$};
    \draw[dashed,red] (0,0,0) -- (-0.5,1.5,0) -- (-0.5,1.5,2);
    \draw[->,>=*]    (-0.5,1.5,2) -- node[above=-2] {\footnotesize $\theta$} (-0.5,2.5,2) node[right=-1] {\small $\bar{r}$};
    ...
    \node[align=center] at (2,1) {$\bar{x} = \bar{r}*R_z(\theta)*\bar{c}$ \\ Revolute};
  \end{tikzpicture}
  \hfill
  \caption{Joint types}
...
\begin{figure}[ht]
  \begin{tikzpicture}[auto, >=latex']
    ...
    \node (type) [function, below of=foreach, node distance=.75cm] {try each joint type (see Alg. \ref{alg:manip-learn})};
    \node (bic) [function, below of=type, node distance=1.4cm, text width=3cm] {choose joint with lowest AIC};
...
\begin{algorithm}[h]
  ...
  \KwIn{$a,b$ tree edge, $\bar{x}_{1..T}^{a,b}$ object part positions}
  ...
  $\bar{\Delta}_{1..T} \leftarrow \bar{x}_{1..T}^b * (\bar{x}_{1..T}^a)^{-1}$ \;
  \For{$q$ \KwInn \{rigid, prismatic, revolute\}}{
    $i, j, k \leftarrow$ \Rand{$1..T$} \;
    \Switch{q}{
      \Case{rigid}{
        $\hat{\theta} \leftarrow \{\bar{\Delta}_i\}$ \;
      }
      \Case{prismatic}{
        $\hat{\theta} \leftarrow \{\bar{\Delta}_i, \frac{T^{-1}(\bar{\Delta}_k - \bar{\Delta}_i)}{||T^{-1}(\bar{\Delta}_k - \bar{\Delta}_i)||}\}$ \;
      }
      \Case{revolute}{
        $P \leftarrow$ \FitPlane{$T^{-1}(\bar{\Delta}_{1..T})$} \;
        $\vec{\delta}_i, \vec{\delta}_j, \vec{\delta}_k \leftarrow$ \Project{$P$, $\bar{\Delta}_{\{i,j,k\}}$} \;
        $\vec{c} \leftarrow$ \Unproject{$P$, \Circumcenter{$\vec{\delta}_{\{i,j,k\}}$}} \\
        $\bar{c} \leftarrow T(\vec{c})*\mat{c}{\vec{P}_{d1} \\\hline \vec{P}_{d2} \\\hline \vec{P}_{d1} \times \vec{P}_{d2}}$ \;
        $\hat{\theta} \leftarrow \{\bar{c}, \bar{\Delta}_i * \bar{c}^{-1}\}$ \;
      }
    }
    $(\theta_q, c_q) \leftarrow$ \Optimize{$\hat{\theta}$, $\lambda \theta .~ ||fk_q(\theta, ik_q(\bar{\Delta}, \theta)) - \bar{\Delta}||^2$}\; \label{line:optimize}
  }
  $J \leftarrow \displaystyle\min_q c_q$ \;
  $\sigma_{1..T} \leftarrow ik_J(\bar{\Delta}_{1..T}, \theta_J)$ \;
...
\end{align}
In this tuple, similar to that of Eq. \ref{eqn:m-tuple}, $a$ and $b$ specify the object parts (graph nodes) connected by the joint, $J$ is the joint type configured by $\theta$, and $\sigma$, the joint state, varies between $\sigma_{min}$ to $\sigma_{max}$ during the simulation.
...
\begin{algorithm}[h]
  ...
  \label{alg:simulate}
  \KwIn{$S$ kinematic tree (see Eq. \ref{eqn:s-tuple}), $(\epsilon_R, \epsilon_T)$ noise amplitude}
  \KwOut{$\bar{x}_{1..T}^{1..K}$ object part trajectories}
  ...
  $\bar{x}_1^1 \leftarrow$ \T{\Sample{$\pm 5$}} $*$ \R{\Sample{$\pm 2\pi$}} \;
  \For{$t \leftarrow 1$ \KwTo $T$}{
    $\bar{x}_t^{1..K} \leftarrow$ \PlaceObjects{$\bar{x}_t^{1..K}$, $S$, 1} \;
    $\overbar{\change x} \leftarrow$ \T{\Sample{$\pm \epsilon_T$}} $*$ \R{\Sample{$\pm \epsilon_R$}} \;
    $\bar{x}_t^{1..K} \leftarrow \overbar{\change x} * \bar{x}_t^{1..K}$ \label{line:dx} \;
    $\bar{x}_{t+1}^1 \leftarrow \bar{x}_t^1 * \overbar{\change x}$ \label{line:dx-root} \;
...
\begin{function}[h]
  ...
  \label{alg:place}
  \KwIn{$\bar{x}_t^{1..K}$ object part positions, $S$ kinematic tree (see Eq. \ref{eqn:s-tuple}), $p$ parent part index}
  \KwOut{$\bar{x}_t^{1..K}$ updated part positions}
  \SetKwFunction{PlaceObjects}{PlaceObjects}
  \DontPrintSemicolon
  \BlankLine
  ...
  \For{$j \leftarrow 1$ \KwTo $N$}{
    \If{$a^j = p$}{
      $\bar{x}_t^{b^j} \leftarrow fk_{J^j}(\theta^j, \sigma^j) * \bar{x}_t^{a^j}$ \;
      $\bar{x}_t^{1..K} \leftarrow$ \PlaceObjects{$\bar{x}_t^{1..K}$, $S$, $b^j$} \;
    }
  }
  \Return $\bar{x}^{1..K}_t$
\end{function}
...
\begin{figure}[ht]
  \centering
  %\includegraphics[width=0.8\textwidth]{img/exp1.png}
...
The matching works well in most cases. Figure \ref{fig:exp2} shows a freeze frame from  (source videos available online). The blue arrows show the relative transformations $\Delta_{1..T}$, and various joint parameters are labeled.

\begin{figure}[ht]
  \centering
  %\framebox[0.4\textwidth]{\raisebox{0pt}[0.07\textwidth][0.07\textwidth]{Visualizations coming soon}}
  \includegraphics[width=.4\textwidth,clip,trim=3.5in 2.5in 2.5in 2.5in,angle=8.5]{img/exp2.png}
  \vspace{-.4in}
  \caption{Freeze-frame from Experiment 2 with detected kinematics. Blue arrows show the axes of rotation.}
  \label{fig:exp2}
\end{figure}
