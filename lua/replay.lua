...
local camdata = utils.try(yaml.loadfile, {string.format('%s/../macbook_air_webcam.yml', params.DATA_DIR)}, 'find camera data YAML')
...
local w
for f = 1,X:size(1) do
    local I = utils.try(image.load, {string.format('%s/%s_dirty_%d.jpg', params.DATA_DIR, dataset, idxs[f]),
                                     string.format('%s/%s_dirty_%d.png', params.DATA_DIR, dataset, idxs[f])},
                        'find the images')
    for i = 1,X:size(2) do
        print(torch.squeeze(X[{ f,i, {},{} }]))
        local p = camdata.camera_matrix
                * torch.eye(3,4)
                * torch.squeeze(X[{ f,i, {},{} }])
