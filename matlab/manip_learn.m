function S = manip_learn(X, dbg)
    ...
    for a = 1:n
        for b = a+1:n
            ...
            [e,g] = jointfit(deltas, [-ct cr, ct-t1 r1], @mex_forward_revolute, @mex_inverse_revolute, @(p) unpack_revolute(p, dims), @gen_jacobians, @gen_jacobians);
            dbg('\t\tinitial revolute objective: %g, %s\n', e, mat2str(g, 3));
